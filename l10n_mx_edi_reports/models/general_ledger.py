...
class MxReportJournalEntries(models.AbstractModel):
    ...
    def get_reports_buttons(self):
        ...
        buttons = super(MxReportJournalEntries, self).get_reports_buttons()
        buttons += [
            {'name': _('Export For SAT (XML)'), 'action': 'print_xml'}]
        return buttons

    def get_templates(self):
        templates = super(MxReportJournalEntries, self).get_templates()
        templates['main_template'] = 'l10n_mx_edi_reports.template_general_ledger_report'  # noqa
    ...
    def _get_lines_third_level(self, move):
        lines = []
        basis_account_ids = [
            account['cash_basis_base_account_id'][0] for account in self.env[
                'account.tax'].read_group([
